...
def minimax(pos_gato, pos_raton, profundidad, es_maximizando, f, c):
    ...
    if pos_gato == pos_raton:
        return -100 if es_maximizando else 100
    ...
    if es_maximizando:
        ...
        for mov in movimientos(f, c, pos_raton):
            valor = minimax(pos_gato, mov, profundidad - 1, False, f, c)
        ...
        for mov in movimientos(f, c, pos_gato):
            valor = minimax(mov, pos_raton, profundidad - 1, True, f, c)
...
while not juego_terminado(gato_pos, raton_pos, turno, max_turnos):
    ...
    turno += 1
    time.sleep(0.5)  # Para poder ver el movimiento
